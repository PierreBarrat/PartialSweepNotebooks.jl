...
begin
	bf_vals = [
		(b=1, f=1),
		(b=0.9, f=0.8), # 2 --> 1
		(b=0.8, f=0.75), # 3 --> all previous ones
	]
	K = let
		K = rand(N,N)
...
let 
	...
	hline!([(1-K[j,i])/(2-K[i,j]-K[j,i])], color = 1, line = :dash, label="")

	# Relative freq of 3/2
	i,j = (2,3)
	f_rel = f[j] ./ (f[i] + f[j])
	plot!(tvals, f_rel, label="Freq. $j vs $i", color = 2)
	hline!([(1-K[j,i])/(2-K[i,j]-K[j,i])], color = 2, line = :dash, label="")

	# Relative freq of 3/1
	i,j = (1,3)
	f_rel = f[j] ./ (f[i] + f[j])
	plot!(tvals, f_rel, label="Freq. $j vs $i", color = 3)
	hline!([(1-K[j,i])/(2-K[i,j]-K[j,i])], color = 3, line = :dash, label="")
	p
end
